...
import { Card, Button, Space, Tag, Badge, Spin, Empty, message } from 'antd';
import { ClearOutlined, PlayCircleOutlined, PauseCircleOutlined, DownloadOutlined, ThunderboltOutlined } from '@ant-design/icons';
...
const SchedulerLogs: React.FC = () => {
  ...
  const triggerScheduler = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/logs/trigger-scheduler', {
        method: 'POST',
      });
      const data = await response.json();
      
      if (data.success) {
        message.success('Scheduler manuel olarak tetiklendi! Logları izleyin...');
      } else {
  ...
  return (
    <PageContainer
      ...
      extra={[
        <Badge key="status" status={isConnected ? 'processing' : 'error'} text={isConnected ? 'Bağlı' : 'Bağlantı Kopuk'} />,
      ]}
    >
      <Card
            ...
            logs.map((log, index) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                  <span style={{ color: '#666', fontSize: '11px', minWidth: '160px' }}>
                    {log.timestampTR || new Date(log.timestamp).toLocaleString('tr-TR', { timeZone: 'Europe/Istanbul' })}
                  </span>
                  <Tag 
                    color={getTagColor(log.type)} 
                    style={{ margin: 0, minWidth: '70px', textAlign: 'center' }}
                  >
                    {log.type.toUpperCase()}
                  </Tag>
                  <span style={{ color: '#ddd', flex: 1, wordBreak: 'break-word' }}>
                    {log.message}
                  </span>
                </div>
