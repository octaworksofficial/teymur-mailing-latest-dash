...
const SPECIAL_DAYS: {
  ...
  {
    value: 'sevgililer_gunu',
    label: 'Sevgililer Günü (14 Şubat)',
    category: 'Özel Günler',
  },
  {
    value: 'kadinlar_gunu',
    label: 'Kadınlar Günü (8 Mart)',
    category: 'Özel Günler',
  },
  {
    value: 'ogretmenler_gunu',
    label: 'Öğretmenler Günü (24 Kasım)',
    category: 'Özel Günler',
  },
...
const SchedulePicker: React.FC<SchedulePickerProps> = ({ value, onChange }) => {
  const [activeTab, setActiveTab] = useState<ScheduleType>(
    value?.schedule_type || 'custom_date',
  );
  ...
  const [intervalValue, setIntervalValue] = useState<number>(
    value?.recurrence_config?.interval || 1,
  );
  const [weekdays, setWeekdays] = useState<number[]>(
    value?.recurrence_config?.weekdays || [1],
  );
  ...
  const [recurrenceTime, setRecurrenceTime] = useState<Dayjs | null>(
    value?.recurrence_config?.time
      ? dayjs(value.recurrence_config.time, 'HH:mm')
      : dayjs('09:00', 'HH:mm'),
  );
  const [endDate, setEndDate] = useState<Dayjs | null>(
    value?.recurrence_config?.end_date
      ? dayjs(value.recurrence_config.end_date)
      : null,
  );
  ...
  const [customSpecialDate, setCustomSpecialDate] = useState<Dayjs | null>(
    value?.special_day_config?.custom_date
      ? dayjs(value.special_day_config.custom_date)
      : null,
  );
  const [customName, setCustomName] = useState<string>(
    value?.special_day_config?.custom_name || '',
  );
  const [dayOffset, setDayOffset] = useState<number>(
    value?.special_day_config?.day_offset || 0,
  );
  const [specialTime, setSpecialTime] = useState<Dayjs | null>(
    value?.special_day_config?.time
      ? dayjs(value.special_day_config.time, 'HH:mm')
      : dayjs('09:00', 'HH:mm'),
  );
  ...
  // İlk render'ı takip et - ilk render'da onChange çağırma
  ...
  useEffect(() => {
          ...
          special_day_config: {
            ...
            custom_date:
              specialDayType === 'custom'
                ? customSpecialDate?.format('YYYY-MM-DD')
                : undefined,
    ...
    specialTime,
    yearlyRepeat,
    // onChange kasıtlı olarak çıkarıldı - sonsuz döngüyü önlemek için
  ]);
  ...
  const groupedSpecialDays = SPECIAL_DAYS.reduce(
    (acc, day) => {
      if (!acc[day.category]) {
        acc[day.category] = [];
      }
          ...
          <Space direction="vertical" style={{ width: '100%' }} size="middle">
            ...
            <Row gutter={16}>
              <Col span={24}>
                ...
                >
                  {Object.entries(groupedSpecialDays).map(
                    ([category, days]) => (
